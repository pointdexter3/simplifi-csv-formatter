...
#  ./rbc-visa-csv-export.bash rbc.csv "2023-12-20"
...
awk -F',' -v date_col=3 'BEGIN{OFS=","} {split($date_col, date_arr, "/"); $date_col = sprintf("%04d-%02d-%02d", date_arr[3], date_arr[1], date_arr[2]); print}'\
...
# with
# $3,$5,"$7","from RBC VISA"
sed -E 's/(.*),(.*),(".*"),(.*),(.*),(.*),(.*),(.*),(.*)/\3,\5,"\7","from RBC VISA"/g' $filename >$filename.tmp && mv $filename.tmp $filename
